    ...
    try:
        ...
        for i in range(rows):
            for j in range(columns):
                # white blocks
                if (np.array_equal(img[7+(i*13), 7+(j*13)], [255, 255, 255])):
                    grid[i][j] = 0
                    # start -> orange block
                elif (np.array_equal(img[7+(i*13), 7+(j*13)], [39, 127, 255])):
                    grid[i][j] = 2
                    # end -> pink block
                elif (np.array_equal(img[7+(i*13), 7+(j*13)], [201, 174, 255])):
                    grid[i][j] = 3
...
def plotter(grid,route):
    ...
    plt.savefig('Map_route_'+number+'.png')
# -----------------------------------------------------
# -----------------------------------------------------
...
number = raw_input('\nEnter Map number for path planning')
print 'Map selected ' + number
time.sleep(1)
print
gray = cv2.imread('maps/'+number+'.png')
...
np.save('Route.npy', route_path)

plotter(grid_map(gray),route_path)

confirm = raw_input('send move commands to robot? y/n')
if confirm == 'y' :
    import Create_route
